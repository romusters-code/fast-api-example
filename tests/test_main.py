...
def test_embed_text(text_input):
...
class TestCalculateSimilarity(unittest.TestCase):
    ...
    def test_calculate_similarity(self, mock_similarity):
        ...
        mock_similarity.return_value = 0.95  # Mock the return value of the similarity function

        # Act
        response = self.client.post("/similarity", json={"text_1": text_1.model_dump(), "text_2": text_2.model_dump()})
        ...
        mock_similarity.assert_called_once_with(text_1="Dog", text_2="Cat")  # Check if the handler was called with the correct parameters
